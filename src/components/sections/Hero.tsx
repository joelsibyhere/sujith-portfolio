      ...
      <div className="site-container relative z-10">
        <div className="grid items-center gap-12 md:grid-cols-[1.2fr_1fr] md:gap-16 lg:gap-24">
          ...
          <Reveal delay={200} className="w-full relative">
            <div className="w-full max-w-[440px] mx-auto md:ml-auto md:mr-0 aspect-[4/5] overflow-hidden rounded-sm bg-muted/10 relative">
              <div className="absolute inset-0 bg-primary/5 mix-blend-overlay z-10 pointer-events-none"></div>
              <img
                src={heroImage}
                alt="Sujith Sreedhar"
                fetchPriority="high"
                className="h-full w-full object-cover object-center saturate-[0.8] contrast-[1.1] transition-transform duration-[2000ms] hover:scale-105"
              />
            </div>
            
            {/* Artistic Stat Badge */}
            <Reveal delay={400} className="absolute -bottom-6 -left-2 md:-left-12 bg-background p-6 border-l-2 border-primary shadow-2xl">
              <p className="text-4xl md:text-5xl font-light tracking-tighter text-foreground mb-1">600+</p>
              <p className="text-[10px] md:text-xs uppercase tracking-[0.2em] text-muted-foreground">Film Credits</p>
            </Reveal>
          </Reveal>
