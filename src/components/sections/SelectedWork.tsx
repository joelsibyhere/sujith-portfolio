...
export function SelectedWork() {
  // Use the 59 optimized posters generated by Codex instead of the database placeholders
  let pool = [...posterGallery];
  
  // Ensure the pool is large enough to span massive horizontal rows without gaps
  if (pool.length > 0) {
    while (pool.length < 150) {
      pool = [...pool, ...posterGallery];
    }
  }

  // Safely slice items for each row from our massive pool (e.g., 25 items per row)
  const row1 = pool.slice(0, 25);
  const row2 = pool.slice(25, 50);
  const row3 = pool.slice(50, 75);
  const row4 = pool.slice(75, 100);
  const row5 = pool.slice(100, 125);
  const row6 = pool.slice(125, 150);
...
function MarqueeItem({ movie }: { movie: typeof posterGallery[0] }) {
  return (
    <div className="flex-none w-[90px] sm:w-[110px] md:w-[140px] lg:w-[170px] aspect-square relative group overflow-hidden bg-card/20 rounded-sm">
      <Link 
        to="/work/$slug" 
        params={{ slug: movie.projectSlug || movie.id }}
